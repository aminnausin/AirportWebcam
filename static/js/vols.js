define('i18n_datatables',[], function () {


    var i18n = ADM.i18n.datatables;
    ...
    return Datatablesi18n;
});
define('datalayer_flight',[
    'UTILS',
    'UTILS_DATE',
    'UTILS_TIME'
], function (Utils, DateUtils, TimeConvertor) {




    function Flight(opts) {
        this.plannedTimestamp = opts.planned || "";
        this.planned = opts.planned_date || "";
        this.plannedHours = opts.planned_hour|| "";
        this.revised = opts.revised_date ||  "";
        this.revisedHours =  opts.revised_hour || "";
        this.revisedTimestamp = this.revised != "" && this.revisedHours && opts.revised || "";
        this.company = opts.company || ""; // logo path (need to do concordance)
        this.company_logo = opts.company_logo_reduce || ""; // logo path (need to do concordance)
        this.company_logo_retina = opts.company_logo_retina || "";// logo path (need to do concordance)
        this.destination = opts.destination || "";
        this.destination_country_code = opts.destination_country_code || "";
        this.destinationWithoutAccent = Utils.normalizeCharacter(this.destination) || "";
        this.flightNumber = opts.flight || "";
        this.statusCode = opts.note || "";
        this.status = opts.details.status || "";
        this.alertSmsUrl = opts.lipso_url || "";
        this.day_filter = opts.day_filter || "";



        var details = opts.details || null,
            waitingtime = details ? details.waitingtime : {},
            canceledInfos = details ? details.canceledInfos : {};

        this.details = {
            hours: details.hours || "",
            hoursRevised: details.hoursRevised || "",
            status: details.status || "",
            statusMore: details.statusMore || "",
            doorNumber: details.doorNumber || "",
            waitingtime: {
                label: waitingtime && waitingtime.label || "",
                duration: waitingtime && waitingtime.duration || ""
            },
            canceledInfos: {
                label: canceledInfos && canceledInfos.label || "",
                phoneNumber: canceledInfos && canceledInfos.phoneNumber || "",
                freePhoneNumber: canceledInfos && canceledInfos.freePhoneNumber || "",
                website: canceledInfos && canceledInfos.website || ""
            }
        };
    ...
    function Flights(data) {
        ...
        var flights = {
                update: {
                    client: {now : Utils.getTimestamp(new Date())},
                    server: {}
                },
                status: {}
            },
        ...
        function deserialize() {
            ...
            // TODO  :: NORMAL CALL:: REMOVE THE CODE WHEN THE REST API IS READY
            /* var d2  = new Date(new Date().getTime() + TimeConvertor.getMilisecondsToMinutes(3));

             flights.update.server.now =  Utils.getTimestamp(new Date());
             flights.update.server.next = Utils.getTimestamp(d2);
             flights.status.code = 200;
             flights.status.message = ""; */


            //  TODO :: FAKE ERROR :: REMOVE THE CODE WHEN THE REST API IS READY
            /*flights.update.server.now =  1387479703;
             flights.update.server.next = 0;
             flights.status.code = 409;
             flights.status.message = "Failure of the last attempt. The service update is temporarily unavailable.";
             */

            // END OF THE REMOVE
            // TODO :: Uncomment line below when the REST API is ready
            var info = data.info;

            flights.update.server.now =  (info && info.timing && typeof info.timing.last !== "undefined") ? info.timing.last : "";
            flights.update.server.next =  (info && info.timing && typeof info.timing.next !== "undefined") ? info.timing.next : "";
            flights.status.code = (info && info.status && typeof info.status.code !== "undefined") ? info.status.code : "";
            flights.status.message = (info && info.status && typeof info.status.message !== "undefined") ? info.status.message : "";


            for(var i = 0; i < length ; i++) {                

                var currentData = data.data[i],
                    timestamp = currentData.planned,
                    cDate,
                    areToday,
                    areTomorrow;
                ...
                if (timestamp !== "") {
                    ...
                    //No more check as client wants us to display everything 
                    /*cDate = DateUtils.convertTimestampToDate(timestamp);
                    areToday = DateUtils.areSameDate(today, cDate);
                    areTomorrow = DateUtils.areSameDate(tomorrow, cDate);

                    if (areToday || areTomorrow) {
                        flightsDT.push(new Flight(data.data[i]));
                    }*/

                    flightsDT.push(new Flight(data.data[i]));
                }
            }
            flights.flights = flightsDT;
        }
...
define('widgets/flightBoardTemplating',[
    'i18n_general'
	 ...
	 *    } );
        ...
        var DataTable = function( oInit )
        {
            ...
            function _fnFeatureHtmlFilter ( oSettings )
            {
                ...
                nFilter.innerHTML = '<label for="search" class="searchLabel">'+sSearchStr+'</label>';
...
define('dataTableWithUtils',[
    'jquery',
    'gsap',
    'dataTable',
    'datalayer_flight'
], function ($,Tweenlite, Flights) {


    /********************************** CUSTOM CLASSES *****************************/

    $.fn.dataTableExt.oJUIClasses.sSortJUI = "icon icon-arrow-updown icon-sortable-desactive";
    $.fn.dataTableExt.oJUIClasses.sSortJUIAsc = "icon icon-arrow-up icon-sortable-asc";
    $.fn.dataTableExt.oJUIClasses.sSortJUIDesc = "icon icon-arrow-down icon-sortable-desc";
    $.fn.dataTableExt.oJUIClasses.sFilter = "dataTables_filter searchDataTables";
    ...
    $.extend( $.fn.dataTableExt.oPagination, {
        "bootstrap": {
            "fnMovePager": function(nPaging) {
                //TODO :: Use TweenLite to move the ctn to
                var $pager = $(nPaging),
            ...
            "fnInit": function( oSettings, nPaging, fnDraw ) {
                ...
                CONFIG = { triggeredEvent: $(oSettings.nTable).data("flightboardsortable-triggeredevent") || null },
                ...
                $(nPaging).addClass('pagination').append(
                    '<ul class="unstyled-list pagination datatables-pagination">'+
                        '<li class="prev disabled pagination-prev"><a href="#" class="btn-primary btn-action icon-inline icon-arrow-right"></a></li>'+
                        '<li class="pagination-listnumber-ctn"><ul class="unstyled-list pagination-listnumber"></ul></li>' +
                        '<li class="next disabled pagination-next"><a href="#" class="btn-primary btn-action icon-inline icon-arrow-left last"></a></li>'+
                        '</ul>'
                );
                ...
                $(window).bind("resize", function() {
                    ...
                    _t.fnUpdate( oSettings, fnDraw );
                });
            },

            "fnUpdate": function ( oSettings, fnDraw ) {
                var oPaging = oSettings.oInstance.fnPagingInfo();
                var iListLength = oPaging.iTotalPages ;
                var an = oSettings.aanFeatures.p;
                var i, j, sClass, bClass, iStart, iEnd, iHalf=Math.floor(iListLength/2);

                var $pager = $(an),
                    $pageNumbers = $pager.find(".pagination-listnumber");
                    $pageNumbersCtn = $pager.find(".pagination-listnumber-ctn");
                ...
                for ( i=0, iLen=an.length ; i<iLen ; i++ ) {
                   ...
                   // $('li:gt(0)',$pageNumbers).filter(':not(:last)').remove();

                    $pageNumbers.empty();

                    // Add the new list items and their event handlers
                    for ( j=iStart ; j<=iEnd ; j++ ) {
                        sClass = (j==oPaging.iPage+1) ? 'pagination-number active' : 'pagination-number';
                        bClass = (j==oPaging.iPage+1) ? 'btn-primary btn-action dark' : 'btn-primary btn-action';

                        if(j==iStart){
                            $('<li class="'+sClass+'"><a href="#" class="' + bClass+ '">'+j+'</a></li>')
                    ...
                    if ( oPaging.iPage === 0 ) {
                        $('li:first', an[i]).addClass('pagination-disabled');
                        $('li:first', an[i]).find(".btn-primary").addClass('neutral');
                    } else {
                        $('li:first', an[i]).removeClass('pagination-disabled');
                        $('li:first', an[i]).find(".btn-primary").removeClass('neutral');
                    }

                    if ( oPaging.iPage === oPaging.iTotalPages-1 || oPaging.iTotalPages === 0 ) {
                        $('li:last', an[i]).addClass('pagination-disabled');
                        $('li:last', an[i]).find(".btn-primary").addClass('neutral');
                    } else {
                        $('li:last', an[i]).removeClass('pagination-disabled');
                        $('li:last', an[i]).find(".btn-primary").removeClass('neutral');
                    }
                }



                 var $prev = $pager.find(".pagination-prev"),
               ...
               $pager.find(".pagination-number").each(function(index, el) {
                ...
                $pageNumbersCtn.css({"width":btnAvailSpace-remainingSpace});
                ...
                var _t = this;
                _t.fnMovePager($pager);

                // TODO :: Check how to position the pager when we click NEXT or PREV .
...
define("urldecoder", ["jquery"], (function (global) {
    return function () {
        var ret, fn;
        return ret || global.jQuery.url;
    };
}(this)));
...
(function (factory) {
    if ( typeof define === 'function' && define.amd ) {
        ...
        define('jScrollPaneWheel',['jquery'], factory);
...
define('widgets/flightBoardSortable',[
    'jquery',
    'enquire',
    'UTILS',
    'momentWithi18n',
...
], function($, enquire, Utils, momentWithi18n, Generali18n, Datatablesi18n, TimeConvertor, DateUtils, DataProvider, Flights, formatFlightDetails, moment) {
    var eventEmitter,
        i = 0,
        breakpoint = {
            arrival:{
                break1: {numCols:7,mediaqueries: "screen and (min-width: 1256px), screen and (min-width: 950px) and (max-width: 1029px), screen and (min-width: 800px) and (max-width: 949px)"},
                break2: {numCols:6,mediaqueries: "screen and (min-width: 650px) and (max-width: 695px)"},
                break3: {numCols:5,mediaqueries: "screen and (min-width: 1090px) and (max-width: 1250px), screen and (min-width: 550px) and (max-width: 649px)"},
                break4: {numCols:4,mediaqueries: "screen and (min-width: 1050px) and (max-width: 1089px), screen and (min-width: 1030px) and (max-width: 1049px), screen and (min-width: 696px) and (max-width: 799px)"},
                break5: {numCols:5,mediaqueries: "screen and (max-width: 549px)"}
            },
            departure:{
                break1: {numCols:8,mediaqueries: "screen and (min-width: 1256px), screen and (min-width: 950px) and (max-width: 1029px), screen and (min-width: 800px) and (max-width: 949px)"},
                break2: {numCols:7,mediaqueries: "screen and (min-width: 650px) and (max-width: 695px)"},
                break3: {numCols:6,mediaqueries: "screen and (min-width: 1090px) and (max-width: 1250px), screen and (min-width: 550px) and (max-width: 649px)"},
                break4: {numCols:4,mediaqueries: "screen and (min-width: 1050px) and (max-width: 1089px), screen and (min-width: 1030px) and (max-width: 1049px), screen and (min-width: 696px) and (max-width: 799px)"},
                break5: {numCols:5,mediaqueries: "screen and (max-width: 549px)"}
            }
        },
        isDarksite = false,
        mobilePlatforms = ['small','medium', 'mobile'],
    ...
    var isMobile = function(){
        ...
        if(u){
            mobile = ($.inArray(u.getPlatform(), mobilePlatforms) !== -1)? true : false;
        }
        if(u.isIE8OrLower()){
            return false;
        } else {
    ...
    function FlightTableRow(type,data, today, tomorrow) {
        ...
        function getRowDate(timestamp, hours, date, isPlannedDate, haveRevisedDate) {
            var html5Datetime = DateUtils.timestampToHtml5Datetime(timestamp),
                status = "",
                template,
                customClass;
    ...
    function FlightBoardSortableInstance(el) {
        ...
        function getFlightInitFilter() {
            var url = $.url.parse(window.location.href),
                querystring = url.params,
                flightNumber = (querystring && querystring.flightNumber) ? String(querystring.flightNumber) : undefined,
                company = (querystring && querystring.company) ? String(querystring.company) : undefined,
                city = (querystring && querystring.city) ? String(querystring.city) : undefined,
                todayonly = (querystring && querystring.today) ? String(querystring.today) : undefined;

            if (flightNumber) {
                if(todayonly !== undefined){
                    return {"flightNumber": flightNumber, "todayonly":todayonly};
                } else {
                    return flightNumber;
                }
    ...
    function FlightboardSearch(opts) {
        ...
        function createEvents() {
            if(!$("html").hasClass("ie7") && !$("html").hasClass("ie8")){
                ...
                enquire.register("screen and (max-width: 1023px)", matchMedia());

                $(window).bind('resize', function(){
                    if (!throttleTimeout) {
                        throttleTimeout = setTimeout(function(){
                            // Update the layout depending on the layoutType property.
                            update();
    ...
    function FlightboardDateLines(opts){
        ...
        function createEvents(){
            if(!$("html").hasClass("ie7") && !$("html").hasClass("ie8")){
                enquire
                    .register(breakpoints.break1.mediaqueries, matchMedia(breakpoints.break1.numCols))
                    .register(breakpoints.break2.mediaqueries, matchMedia(breakpoints.break2.numCols))
                    .register(breakpoints.break3.mediaqueries, matchMedia(breakpoints.break3.numCols))
                    .register(breakpoints.break4.mediaqueries, matchMedia(breakpoints.break4.numCols))
                    .register(breakpoints.break5.mediaqueries, matchMedia(breakpoints.break5.numCols));
            } else {
                numCol = breakpoints.break1.numCols;
            }
        ...
        function createLines(){
            ...
            $dateList.each(function(){
                ...
                if(isNewDateRow){
                    var thatDateValue = DateUtils.getDayAndMonthName(rowDate);
        ...
        function createEvents() {
            ...
            $el.on("click", ".tableauvols-toggle-button", function(evt) {
                ...
                function expand(){
                    ...
                    dataLayer.push({
                        'event': 'visionnement_vol_' + status + '_' + Utils.getLang(),
...
define('widgets/destinationsDirectes',[
    'jquery',
    ...
    var DestinationsDirectes = {
        init: function (context) {
            eventEmitter = ADM.eventEmitter;
            $(context).each(function(index, el) {
                new DestinationsDirectesInstance(el);
            });
        }
    };


    function DestinationsDirectesInstance(el) {
        console.log("Destinations Directes");

        var $el = $(el),
            CONFIG = { receiverEvent: $el.data("domfilters-receiverevent") };
        ...
        function createEvents() {
            if(CONFIG.receiverEvent) {
                eventEmitter.addListener(CONFIG.receiverEvent, function(eventType,filterType){
                   $el.trigger("footable_untoggleall_row");
                });
            }
 ...
 * Data-attribute:
 *
 * data-domfilters-selector {default: null}         :  The Selector of the elem that you want filter
 * data-domfilters-triggerevent  {default:null}     :  Pass the eventEmitter name that you want to listen event by eventEmitter
 * data-domfilters-receiverevent {default: null}    :  Pass the eventEmitter name that you want to trigger by eventEmitter
 * data-domfilters-filtertype                       : Apply this data-attr on the element that you want try to filter with the filter type.
 *
 * <div data-enhance="domFilters" class="enhance" data-domfilters-selector="country"  data-domfilters-triggerevent="board-filterselected" data-domfilters-receiverevent="board-filtersupdate">
 *     <ul>
 *         <li data-domfilters-filtertype="International" class='country'>Quebec</li>
 *         <li data-domfilters-filtertype="International" class='country'>Alberta</li>
 *         <li data-domfilters-filtertype="International" class='country'>Boston</li>
 *         <li data-domfilters-filtertype="International" class='country'>New York</li>
 *     </ul>
 *
 * </div>
 */
...
define('widgets/domFilters',[
    ...
    var DomFilters = {
        init: function (context) {
            eventEmitter = ADM.eventEmitter;
            $(context).each(function(index, el) {
                new DomFiltersInstance(el);
            });
        }
    };


    function DomFiltersInstance(el) {
        var $el = $(el),
            CONFIG = {
                receiverEvent: $el.data("domfilters-receiverevent") ||  null,
                triggerEvent: $el.data("domfilters-triggerevent") ||  null,
                filterSelector: $el.find($el.data("domfilters-selector")) ||  null
            };
        ...
        function initFilter() {
            if(haveInitFilter()) {
                var params = $.url.parse(window.location.href).params,
                    filterType =  params.destinationType ;

                handleDomFilter(filterType);

                eventEmitter.emitEvent(CONFIG.triggerEvent,["domFilters-filterupdate",filterType]);
            }

            if(!haveInitFilter() && haveInitDefaultFilter()) {
                var filterType = $el.data("domfilters-defaultfilter");

                handleDomFilter(filterType);
            }
        }

        function handleDomFilter(filterType) {
            switch(filterType) {
                case "all":
                    removeDomFilter();
                    break;
        ...
        function haveInitDefaultFilter() {

            return (typeof $el.data("domfilters-defaultfilter") != "undefined")  ? true : false;
        }


        function removeDomFilter() {
            CONFIG.filterSelector.removeClass("hide");
        }

        function domFilter(filterType) {
            removeDomFilter();

            CONFIG.filterSelector.filter("data[domfilters-filtertype!='" + filterType + "']").addClass("hide");

            $.each( CONFIG.filterSelector, function( i, el  ) {
                var $el = $(el),
                    elFilterType = $el.data("domfilters-filtertype");

                if(elFilterType != filterType) {
                    $el.addClass("hide");
                }
            });
        }
...
define('widgets/pages/vols',[
    'jquery',
    'UTILS',
    'widgets/flightBoardSortable',
    'widgets/destinationsDirectes',
    'widgets/domFilters',

    'gsap'

], function ($, Utils, FlightBoardSortable, DestinationsDirectes, DomFilters) {

    var Widgets = {
        flightBoardSortable: function(context) {
            FlightBoardSortable.init(Utils.removeHighlightedCodeFromContext(context));
        },
        destinationsDirectes: function(context){
            DestinationsDirectes.init(context);
        },
        domFilters: function(context) {
            DomFilters.init(context);
        }
    };
...
require([
    ...
    require([
        ...
        'enhance'
    ], function ($, Utils, Widgets) {

        // Compatibility test
        if(Utils.compatibilityTest()) {


            $.enhance(Widgets.flightBoardSortable, { id: "flightBoardSortable", title: "Filters Group"});
            $.enhance(Widgets.destinationsDirectes, {id: "destinationsDirectes", title: "Comments Form.", emitter: ADM.eventEmitter});
            $.enhance(Widgets.domFilters, {id: "domFilters", title: "Filters Group"});

        }

        function remove(arr, what) {
            var found = $.inArray(what,arr);
        ...
        console.log("dependanciesToBeLoaded", dependanciesToBeLoaded);
        remove(dependanciesToBeLoaded, 'vols');
        console.log("dependanciesToBeLoaded vols removed", dependanciesToBeLoaded);

        // fix a bug in static
        if(ADM.eventEmitter === null || typeof ADM.eventEmitter === 'undefined'){
            setTimeout(function(){
                if(ADM.eventEmitter === null || typeof ADM.eventEmitter === 'undefined'){
                    setTimeout(function(){
                        ADM.eventEmitter.emitEvent('dependancyLoaded');
                    }, 1000);
                } else {
                    ADM.eventEmitter.emitEvent('dependancyLoaded');
                }
